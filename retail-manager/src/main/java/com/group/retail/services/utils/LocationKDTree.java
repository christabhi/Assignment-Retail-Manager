...
public class LocationKDTree {
    ...
    public LocationKDTree(@Nonnull final List<Shop> shopList) {
        final List<Node> nodes = new ArrayList<>(shopList.size());
        for (final Shop shop : shopList) {
            nodes.add(new Node(shop));
        }
        tree = buildTree(nodes, 0);
    }

    @Nullable
    public Shop findNearest(final double latitude, final double longitude) {
        final Node node = findNearest(tree, new Node(latitude, longitude), 0);
        return node == null ? null : node.nearestShop;
    }
    ...
    private static class Node {
        ...
        Node(final Shop shop) {
            this(shop.shopLatitude, shop.shopLongitude);
